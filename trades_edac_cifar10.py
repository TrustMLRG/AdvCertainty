...
parser.add_argument('--model-dir', default='./checkpoints/trades_edac_cifar10',
...
# settings
model_dir = f'{args.model_dir}_{args.model}_{str(args.stepsize)}'
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        """
        Optimizing Adversarial Certainty
        theta_t -> theta_(t+0.5)
        """

        _, x_adv = trades_loss(model=model,
                           x_natural=data,
                           y=target,
                           optimizer=optimizer,
        ...
        robust_std = torch.std(robust_output, dim=1)
        advCertainty = robust_std.mean()

        total_loss = args.stepsize * advCertainty

        total_loss.backward()
        optimizer.step()

        """
        Optimizing Adversarial Robustness
        theta_(t+0.5) -> theta_(t+1)
        """

        optimizer.zero_grad()

        loss, _ = trades_loss(model=model,
                           x_natural=data,
                           y=target,
                           optimizer=optimizer,
                           step_size=args.step_size,
